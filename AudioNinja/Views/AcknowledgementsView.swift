...
/// with the licence text and to say where the library's source is, and LAME asks that its use be
/// acknowledged with a link to the project. A licence file in the repository reaches nobody who
/// installs from the App Store.
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text("LAME")
                    .font(.title2.bold())
                Text("""
                    Audio Ninja uses LAME 3.100 to encode MP3 files. LAME is free software, \
                    distributed under the GNU Library General Public License, version 2 or \
                    (at your option) any later version. It is included unmodified, as a \
                    separate dynamic library (LAME.framework inside the app).
                    """)
                Link("lame.sourceforge.io", destination: URL(string: "https://lame.sourceforge.io/")!)
                // The exact source the shipped framework was built from, with its build scripts.
                Link(
                    "Source code for the included LAME.framework",
                    destination: URL(string: "https://github.com/BB9z/LAME-xcframework/tree/3.100.3")!
                )
